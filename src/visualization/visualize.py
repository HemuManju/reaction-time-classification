...
def plot_detection_false_alarm(config):
    ...
    features = [
        'detection_percent_av', 'false_detection_av', 'performance_level'
    ]
    ...
    sb.scatterplot(x=df[features[0]],
                   y=df[features[1]],
                   style=df[features[2]],
                   markers=markers,
                   hue=df[features[2]])
...
def plot_reaction_time(subject, config):
    ...
    pdf_invgauss = stats.invgauss.pdf(lnspc,
                                      mu=result[0],
                                      loc=result[1],
                                      scale=result[2])
